...
public class StringUtils {
     ...
     * 提取字符串中最后一组数字
     */
    public static String lastNumStringAdd(String src) {
        ...
        int numStartPosition = 0;
        int numEndPosition = 0;
        //resolve last num array
        ...
        //add 1
        String addResult = numStringAdd(num.toString());
        //merge
        StringBuilder result = new StringBuilder();
        char[] charStart = new char[numStartPosition];
        src.getChars(0, numStartPosition, charStart, 0);
        result.append(charStart);
        result.append(addResult);
        if (numEndPosition < src.length()) {//判断数字之后是否还有字母
            char[] charEnd = new char[src.length() - numEndPosition];
            src.getChars(numEndPosition + 1, src.length(), charEnd, 0);
            result.append(charEnd);
        }
        return result.toString();
    }

    private static String numStringAdd(String src) {
        try {
            long num = Long.valueOf(src);
            return num + 1 + "";
     ...
     * 提取字符串中最后一组数字
     */
    public static String lastNumStringReduce(String src) {
        if (src == null || "".equals(src)) {
            return "0";
        }
        src = src.trim();
        ...
        //add 1
        String addResult = numStringReduce(num.toString());
        //merge
        StringBuilder result = new StringBuilder();
        char[] charStart = new char[numStartPosition];
        src.getChars(0, numStartPosition, charStart, 0);
    ...
    private static String numStringReduce(String src) {
        try {
            long num = Long.valueOf(src);
            return num - 1 + "";
        } catch (NumberFormatException e) {
     ...
     * @param start        起始字符串 例如：<tvWaybill>
     * @param end          结束字符串 例如：</tvWaybill>
     * @param defaultValue
     * @return
     */
     ...
     * @return
     */
    public static String generateTimeBySecond(long time) {
